...
\urlstyle{same}
\hypersetup{
  pdftitle={The Attention Cycle: Supervision Tax{,} Executable Documents{,} and Protected Attention},
  pdfauthor={Leric Zhang},
...
\title{The Attention Cycle: Supervision Tax, Executable Documents, and
Protected Attention}
\author{Leric Zhang}
\date{}
\begin{document}
...
its durable outputs accumulate, executable task documents are the
runnable subset of that substrate, and review is where human judgment
re-enters without continuous monitoring.
...
off-the-shelf components: any document workspace (Notion, Obsidian,
GitHub issues, Linear, plain Markdown), any conversational agent capable
of editing that workspace, and any execution-oriented agent capable of
running asynchronously and writing back (Hermes, OpenClaw, Codex-style
or Claude Code-style coding agents, browser or shell agents). A
dedicated product designed end-to-end around the cycle is also a
reasonable path; it is not, however, a prerequisite for putting the
