...
re-engaged only at decision boundaries.
\end{abstract}
...
they supply intent, judgment, taste, context, and accountability --- a
role long emphasized in mixed-initiative and human-in-the-loop systems
(Horvitz 1999; Amershi et al. 2014). Removing them does not save labor
--- it removes the source of value.
...
agents in parallel --- increasingly plausible as multi-agent frameworks
mature (Wu et al. 2023; Hong et al. 2024) --- produce ten such streams
converging on the same attention. Parallelism does not absorb
under-specification; it concentrates it into more concurrent surface
area than one human can keep up with:
...
agents. Recent systems and papers make this direction explicit:
planner/executor architectures target long-horizon web tasks, research
agents chain browsing, analysis, and synthesis, frontier coding agents
report sustained tool-using workflows, and stateful workbenches are
emerging for long-running mathematical collaboration (Erdogan et al.
2025; OpenAI 2025; Anthropic 2025; Zheng et al. 2026). The ambition is
to run further on each invocation, recover from more failures,
coordinate more tools, and call back to the human less often. For
routine work, this is genuine progress. For intent-bound work, however,
it raises the bar on what the agent is given to start with. The longer
and more autonomously an agent operates, the more weight its initial
specification has to carry --- every gap, ambiguity, or unstated
constraint becomes either an unsupervised wrong turn or a runtime
callback to the human. \textbf{More-autonomous agents do not weaken the
demand for clarified intent; they sharpen it.}
...
The inspiration comes from \textbf{Getting Things Done} (Allen 2001):
externalize open loops into a trusted system so nothing competes for
attention except what genuinely needs it now. Document-mediated
collaboration extends that move into AI-mediated work --- once intent is
materialized into an executable document, the running task lives in the
document and the agents acting on it, not in the human's head.
...
Figure~\ref{fig:attention-cycle} summarizes the cycle as a single
diagram.
...
into a stable artifact a background agent can act on. It serves as a
coordination artifact in the sense of a boundary object (Star and
Griesemer 1989): a single shared thing that several roles interpret
differently while still treating it as the same thing. It also serves as
an executable specification in the sense developed for agile teams,
where worked examples and acceptance criteria are written so that they
can both be read by humans and run against the system (Adzic 2011).
Concretely, it records the task's intent, context, constraints, accepted
decisions, open questions, definition of done, and later execution
results. It becomes the single source of truth for that piece of work:
the background agent reads from it, the human reviews through it, the
Review perspective critiques it, and execution history accumulates back
into it.
...
\bibitem[\citeproctext]{ref-adzic2011specification}
Adzic, Gojko. 2011. \emph{Specification by Example: How Successful Teams
Deliver the Right Software}. Manning Publications.

\bibitem[\citeproctext]{ref-allen2001getting}
Allen, David. 2001. \emph{Getting Things Done: The Art of Stress-Free
...
\bibitem[\citeproctext]{ref-anthropic2025claude4}
Anthropic. 2025. \emph{Claude Opus 4 and Claude Sonnet 4 System Card}.
...
\bibitem[\citeproctext]{ref-erdogan2025planandact}
Erdogan, Lutfi Eren, Nicholas Lee, Sehoon Kim, et al. 2025.
{``{PLAN-AND-ACT}: Improving Planning of Agents for Long-Horizon
Tasks.''} \emph{arXiv Preprint arXiv:2503.09572}, ahead of print.
\url{https://doi.org/10.48550/arXiv.2503.09572}.

\bibitem[\citeproctext]{ref-hong2024metagpt}
Hong, Sirui, Mingchen Zhuge, Jonathan Chen, et al. 2024. {``{MetaGPT}:
Meta Programming for a Multi-Agent Collaborative Framework.''} \emph{The
Twelfth International Conference on Learning Representations (ICLR)}.

\bibitem[\citeproctext]{ref-horvitz1999principles}
Horvitz, Eric. 1999. {``Principles of Mixed-Initiative User
Interfaces.''} \emph{Proceedings of the SIGCHI Conference on Human
...
\bibitem[\citeproctext]{ref-openai2025deepresearch}
OpenAI. 2025. \emph{Deep Research System Card}.
\url{https://cdn.openai.com/deep-research-system-card.pdf}.

\bibitem[\citeproctext]{ref-star1989institutional}
...
\emph{Social Studies of Science} 19 (3): 387--420.
...
Framework.''} \emph{arXiv Preprint arXiv:2308.08155}.

\bibitem[\citeproctext]{ref-zheng2026comathematician}
Zheng, Daniel, Ingrid von Glehn, Yori Zwols, et al. 2026. {``{AI}
Co-Mathematician: Accelerating Mathematicians with Agentic {AI}.''}
\emph{arXiv Preprint arXiv:2605.06651}, ahead of print.
\url{https://doi.org/10.48550/arXiv.2605.06651}.
