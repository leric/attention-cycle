...
The second half follows from how autonomy scales. One agent on under-specified intent produces a stream of clarification requests, partial outputs, and decision points flowing back to the human. Ten agents in parallel — increasingly plausible as multi-agent frameworks mature [@wu2023autogen; @hong2024metagpt] — produce ten such streams converging on the same attention. Parallelism does not absorb under-specification; it concentrates it into more concurrent surface area than one human can keep up with:
...
Within that substrate, a **task document** is the *executable subset* — the kind of document that turns a clarified slice of intent into a stable artifact a background agent can act on. It serves as a coordination artifact in the sense of a boundary object [@star1989institutional]: a single shared thing that several roles interpret differently while still treating it as the same thing. It also serves as an executable specification in the sense developed for agile teams, where worked examples and acceptance criteria are written so that they can both be read by humans and run against the system [@adzic2011specification]. Concretely, it records the task’s intent, context, constraints, accepted decisions, open questions, definition of done, and later execution results. It becomes the single source of truth for that piece of work: the background agent reads from it, the human reviews through it, the Review perspective critiques it, and execution history accumulates back into it.
